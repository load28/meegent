...
import { ensureLanguage } from "./treesitter.js";
...
export function setupHashlineView(pi: ExtensionAPI): void {
  pi.on("tool_result", async (event, ctx) => {
    ...
    // Warm the tree-sitter grammar so a subsequent `replace block`/`delete block` resolves precisely.
    await ensureLanguage(path);
